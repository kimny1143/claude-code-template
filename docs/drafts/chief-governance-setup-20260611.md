...
| **層2: settings deny** | Bash 変更系 / git 変更系 / Chrome / 外部write MCP / dispatch / secrets read | `_chief/.claude/settings.local.json` permissions.deny |
...
```jsonc
{
  "permissions": {
    "deny": [
      ...
      // --- dispatch (peer への指示出し = 指揮権なし。下記「送信の扱い」参照) ---
...
### ★ send_message (dispatch) の扱い — **deny + file queue 採用確定 (conductor 2026-06-11)**
- settings は送信先で allow/deny を分けられない (send_message は to_id 引数1つ) → recipient 制限は不可。
- **採用 = `mcp__claude-peers__send_message` を deny** し、Chief→conductor は **`_chief/for-conductor/` の file queue** で行う (conductor が patrol / ターン頭で当該 dir を読む)。→ Chief は構造的に**どの peer にも dispatch 不能** = §2.1「dispatch しない」を機構保証 + §5-2「成果物ベース連携」と整合。「構造保証 > 規約」(conductor 判断)。
- ※ path = `_chief/for-conductor/` (workspace 名 = `_chief` で確定。`_strategy` は仮名だった)。
- ※ file queue は Chief cwd 内 (`_chief/for-conductor/`) ゆえ層1 hook の cwd-allow に自然に収まる (別途 allowlist 不要)。
...
4. `_strategy/for-conductor/` queue ディレクトリ作成 (send_message deny 採用時)。
