...
### 7. MCP エージェント設計ガイド (`mcp-agent-design/`)

マルチエージェントチームにおけるMCPサーバーの設計・配置・運用パターン。

- `mcp-design-guide.md` — MCP配置レベル（グローバル/プロジェクト/セッション）、設計パターン（インフラ/ドメイン/ツール）、APIキー管理、権限設計、フック連携、カスタムMCPチェックリスト、排他制御、ライフサイクル管理
