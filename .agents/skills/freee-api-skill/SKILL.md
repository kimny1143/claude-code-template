...
> **リファレンス版数について**: 本スキルの API リファレンス（`references/` 72 件）は取り込み日 **2026-03-31** 時点のスナップショットです。freee API 仕様は更新されうるため、以降の変更は未反映の可能性があります。重要項目は `freee_api_list_paths` ツール／freee 公式ドキュメントで最新を確認してください。
