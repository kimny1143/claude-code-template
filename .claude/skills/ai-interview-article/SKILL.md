...
- kimny（木村篤史）。作曲家・編曲家。glasswerks inc.代表
- J-POP・CMなど作編曲を横断（AKB48、JUJU、ももクロ等）
- 音楽制作ツール群 MUED プロジェクト開発者（MUEDsp / MUEDial / MUEDlim）。MUEDial の対話キャラクター Hoo を展開中
...
*MUED開発者 kimny × Claude（AI）*
```

- タイトルは内容を端的に表す。「〜について」的な説明調は避ける。
- サブタイトルは `*イタリック*` で著者クレジットのみ。
- 将来的にインタビュアーをHooに変更する可能性あり。その場合は `*MUED開発者 kimny × Hoo（AI）*` に。
...
```markdown
> **この記事のポイント:** プロ作曲家は「メロディが降りてくる」経験を持たない。これまでのキャリアで一度もない。創作の正体は「記憶の再構成」であり、インプットの質と量がアウトプットを決める。
```
...
導線は固定しない。記事テーマに最も近いものを1つだけ選ぶ（MUED 製品 / MUEDial / advisory）。

```markdown
---

*〈記事テーマに最も近い導線を1文だけ〉→ [MUED.jp](https://mued.jp)*
```

導線先の選び方（テーマ別に1つ）:
- 音楽制作・DTM 系 → MUED 製品（MUEDsp 等）
- AI・対話・キャラクター系 → MUEDial（対話キャラクター Hoo）
- 事業・設計・AI 運用系 → advisory（設計相談）

- 全記事共通。1〜2文。押し売りしない。
- 記事の内容と導線先の接点がある場合のみ、もう1文追加可
...
- **プロモーション価値** — 記事テーマに応じた MUED 導線（製品 / MUEDial / advisory）への自然な導線（押し売りにしない）
...
### 8. natural-japanese lint（公開前の最終工程）

品質ゲート（公開前）を通過した記事に、公開直前の最終工程として natural-japanese lint を通す。AI 臭のリズム（文長の単調さ＝burstiness・段落の均質性・翻訳調・紋切り型フレーズ）を機械検出する補助ツール。

- **実行**: natural-japanese プラグイン（`/plugin marketplace add coji/natural-japanese`）の `scripts/lint.py` を通す（例: `uv run scripts/lint.py 記事.md`）。
- **工程順**: …→品質ゲート（公開前）→**lint→修正 or 理由付き残置→再 lint で収束確認**→公開。lint は品質ゲートの後に置く（前に置いて執筆を縛らない）。
- **運用ルール:**
  1. 検出は「疑い」であって断定ではない。**機械的に全部直さない。**
  2. 誤検知は理由を添えて残置してよい（残置理由を記録する）。
  3. lint 通過そのものを目的化しない（Goodhart 回避）。
  4. 検出ゼロを公開条件にしない。「重大」判定のみ修正必須。
  5. 禁止語・文体の定義は本スキル側が正。lint は補助検出。
...
9. **lint 検出を機械的に全修正して新しい定型を作る** → lint は「疑い」。全部直すと lint 最適化された別種のAI臭（新しい定型）を生む。重大のみ直し、誤検知は理由付きで残す
